...
当前仓库已完成动作指导和训练记录 MVP：正式应用采用 **Tauri 2 + React + TypeScript**，训练数据使用本机 SQLite 保存。macOS Apple Silicon 的 `.app` 与 `.dmg` 已完成本地构建和完整训练流程验证；Windows 工程配置已经具备，安装包仍需在 Windows 或 CI 构建机上生成和测试。
