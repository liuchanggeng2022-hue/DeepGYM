...
当前仓库已完成动作指导、训练记录 MVP，以及账号与云同步的代码实现：正式应用采用 **Tauri 2 + React + TypeScript**，训练数据先写入本机 SQLite，再通过 Supabase 同步到用户账号。加入账号功能后的 macOS Apple Silicon `.app` 已完成本地编译；真实注册和同步仍需连接 Supabase 项目后联调，Windows 由 CI 编译并在正式发布前真机测试。
