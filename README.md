...
动作文字和数据结构使用 MIT 许可；图片与 GIF 不在 MIT 许可内，版权属于 Gym visual。当前原型从上游仓库远程加载媒体，仅用于本地开发确认，不应直接公开部署或商业发布。正式发布前需由产品所有者阅读 Gym visual 条款，并在需要时取得独立许可。详见 [数据与媒体许可说明](docs/DATA_AND_MEDIA_LICENSE.md)。
